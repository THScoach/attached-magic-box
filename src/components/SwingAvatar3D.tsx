...
// Map joint names to MediaPipe landmark IDs
const JOINT_NAME_TO_ID: Record<string, number> = {
  'nose': 0,
  'left_eye_inner': 1,
  'left_eye': 2,
  'left_eye_outer': 3,
...
function Scene({ poseData, currentTime, duration }: SwingAvatar3DProps) {
  const getCurrentJoints = () => {
    ...
    Object.entries(frame.joints).forEach(([key, joint]) => {
      // Try to map joint name to landmark ID, or parse as number if already numeric
      const landmarkId = JOINT_NAME_TO_ID[key] ?? parseInt(key);
      if (!isNaN(landmarkId) && landmarkId >= 0 && landmarkId < 33) {
    ...
    const validJoints = jointsArray.filter(j => j !== undefined);
    console.log('Valid joints:', validJoints.length);
    return validJoints;
  };
